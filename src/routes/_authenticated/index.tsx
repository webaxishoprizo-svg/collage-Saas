...
import { useDB, useDBStatus, formatINR } from "@/lib/store";
    ...
    >
      ...
      {isLoading ? (
        ...
        <EmptyState
          ...
          description="Add your first worker to start tracking attendance and wages."
          cta={<Link to="/workers/add" className="inline-flex items-center gap-2 bg-primary text-primary-foreground rounded-lg px-4 py-2.5 text-sm font-medium"><Plus className="h-4 w-4" /> Add worker</Link>}
        />
      ) : (
        <div className="space-y-3">
          {db.workers.map((w) => {
